...
- **Микросервисы** на Golang обрабатывают бизнес-логику. Каждый сервис масштабируется независимо под нагрузку (пик до 3 628 RPS).
- **PostgreSQL** (основное хранилище) содержит таблицы `users`, `properties`, `bookings`, `payments`, `reviews`. Для таблиц `reviews` (1,2 ТБ) и `payments` (105 ГБ) используется шардирование через Citus. Резервирование: мастер + 2 реплики, failover через Patroni.
...
- Пометки `r/w` на стрелках означают чтение/запись. В большинстве случаев чтение доминирует (1 968 RPS на `properties` против 20 RPS записи).
...
Все сервисы развёртываются в 5 дата-центрах (DC‑EU1, DC‑EU2, DC‑US1, DC‑AP1) в гибридной инфраструктуре (on‑premises + AWS). Stateless-микросервисы (Golang) работают под управлением Kubernetes; stateful-компоненты (PostgreSQL, Elasticsearch, Redis, Kafka) — на выделенных физических серверах вне оркестрации.

Расчёты выполнены для **DC‑EU1 (Европа)** — наиболее загруженного региона с долей **71,7%** трафика, пиковым трафиком **107,6 Гбит/с** и пиковой нагрузкой **3 628 RPS**. Для остальных ДЦ количество серверов пересчитывается пропорционально доле трафика (см. таблицу распределения в п. 3.3).

### 11.1 Ресурсные требования сервисов (DC‑EU1, пиковая нагрузка)

Нормативы производительности (на ядро CPU):
- Go‑микросервис, лёгкое API (запросы к Redis/кэш) → 5 000 RPS/ядро
- Go‑микросервис, средняя бизнес-логика (запросы к PostgreSQL) → 2 000 RPS/ядро
- Go‑микросервис, работа с Elasticsearch (поиск) → 3 000 RPS/ядро
- HAProxy (L7) с SSL‑терминацией → 6 676 CPS/ядро (из расчёта в п. 4.2)

| Сервис | Пиковая нагрузка (DC‑EU1) | CPU (ядер) | RAM | Примечание |
| :--- | :--- | :--- | :--- | :--- |
| **LVS (L4)** | 107,6 Гбит/с | 4 ядра × 10 серверов = 40 | 40 ГБ | Используется ECMP на коммутаторах, LVS – резервные серверы |
| **HAProxy (L7)** | 3 628 CPS / 107,6 Гбит/с | 8 ядер × 6 серверов = 48 | 48 ГБ | См. расчёт в п. 4.2 |
| **Поиск** | 788 RPS на поиск + 630 RPS на отзывы? Уточнение: поиск – 788 RPS, просмотр отзывов не требует отдельного сервиса? Выделим отдельно. | 788 / 3000 ≈ 1 ядро → с запасом 4 ядра | 8 ГБ | Обращение к Elasticsearch |
| **Бронирования** | 78 RPS (запись) + 78 RPS (чтение) – но чтение может идти через кэш. Примем 200 RPS для страховки | 200 / 2000 ≈ 1 ядро → 4 ядра | 8 ГБ | Чтение/запись PostgreSQL |
| **Платежи** | 78 RPS запись, 78 RPS чтение | 150 / 2000 ≈ 1 ядро → 4 ядра | 8 ГБ | PostgreSQL, ACID |
| **Профили** | 1 814 RPS (чтение) + 61 RPS (запись) – в основном чтение | 1814 / 5000 ≈ 1 ядро → 4 ядра | 8 ГБ | Проверка сессий, чтение users |
| **Отзывы** | 630 RPS чтение, 1 RPS запись | 630 / 3000 ≈ 1 ядро → 4 ядра | 8 ГБ | Чтение из PostgreSQL, запись через Kafka |
| **Рекомендации** | 1 814 RPS чтение из Redis, запись через Kafka | 1814 / 5000 ≈ 1 ядро → 4 ядра | 8 ГБ | Кэш Redis |
| **Интеграционный сервис (Kafka consumer)** | Обновление рейтинга и кэша – 2 300 событий/с (отзывы + бронирования) | 2300 / 5000 ≈ 1 ядро → 2 ядра | 4 ГБ | Асинхронные воркеры |
| **PostgreSQL (кластер)** | 1 968 RPS чтения, 60 RPS записи (максимум) – суммарно ~2 028 QPS | 192 ядра (на весь кластер) | 768 ГБ | 3 шарда × (мастер + 2 реплики) = 9 серверов; на один сервер: 64 ядра, 256 ГБ RAM. Цифры приблизительные. |
| **Elasticsearch** | 788 RPS поиска | 32 ядра | 64 ГБ | Кластер из 3 узлов; каждый узел: 16 ядер, 32 ГБ |
| **Redis** | 1 814 RPS чтения + 1 814 RPS записи (рекомендации) + сессии | 16 ядер | 32 ГБ | Redis Cluster: 3 мастера + 3 реплики, всего 6 узлов |
| **Kafka** | ~2 300 событий/с | 8 ядер | 16 ГБ | 3 брокера, RF=3 |
| **S3 + CDN** | 107,6 Гбит/с исходящего трафика | – | – | Объектное хранилище, не требует CPU в ДЦ |

**Итого CPU для микросервисов в Kubernetes (DC‑EU1):**  
Поиск (4) + Бронирования (4) + Платежи (4) + Профили (4) + Отзывы (4) + Рекомендации (4) + Kafka consumer (2) = **26 ядер**.  
С учётом системных подов и запаса (+30%): **~34 ядра**.

**Конфигурация Kubernetes узлов:**  
Узел: 16 vCPU, 64 ГБ RAM. Количество узлов = ceil(34 / 16) = 3 узла.  
Добавляем 1 резервный узел для отказоустойчивости → **4 узла**.

### 11.2 Конфигурации физических серверов (все ДЦ)

| Название | Хостинг | Конфигурация | Ядра | Кол-во (DC‑EU1) | Кол-во (все ДЦ) | Примечание |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **kubenode** | on‑prem | 2×AMD EPYC 7282 / 4×16 ГБ / 2×NVMe 1 ТБ / 2×25GbE | 16 | 4 | 4 (EU) + 1 (US) + 1 (AP) ≈ 6 | 1 узел на Америку и Азию (малая нагрузка) |
| **lvs-l4** | on‑prem | 1×Intel Xeon E-2334 / 4×8 ГБ / 1×NVMe 256 ГБ / 2×25GbE | 4 | 10 | 10 (EU) + 4 (US) + 4 (AP) = 18 | См. п. 4.3 |
| **haproxy-l7** | on‑prem | 2×Intel Xeon Silver 4314 / 4×16 ГБ / 2×NVMe 512 ГБ / 2×25GbE | 16 | 6 | 6 (EU) + 2 (US) + 2 (AP) = 10 | См. п. 4.3 |
| **db-postgres** | on‑prem | 2×AMD EPYC 7282 / 8×32 ГБ / 4×NVMe 4 ТБ / 2×25GbE | 32 | 9 | 9 (EU) + 3 (US) + 3 (AP) = 15 | Каждый шард (3 шарда) + реплики (2) = 9 на регион; для малых регионов – 3 сервера |
| **db-elasticsearch** | on‑prem | 2×Intel Xeon Silver 4210 / 4×16 ГБ / 4×NVMe 2 ТБ / 2×25GbE | 16 | 3 | 3 (EU) + 1 (US) + 1 (AP) = 5 | RF=2; на малые регионы 1 узел (данные реплицируются в EU) |
| **db-redis** | on‑prem | 2×Intel Xeon Silver 4210R / 8×16 ГБ / 2×NVMe 512 ГБ / 2×25GbE | 16 | 6 | 6 (EU) + 2 (US) + 2 (AP) = 10 | Redis Cluster: 3 мастера + 3 реплики |
| **kafka** | on‑prem | 2×Intel Xeon Silver 4210 / 4×16 ГБ / 4×NVMe 2 ТБ / 2×25GbE | 16 | 3 | 3 (EU) + 1 (US) + 1 (AP) = 5 | RF=3; брокеры в малых регионах могут синхронизироваться с EU |

### 11.3 Аллокация ресурсов в Kubernetes (DC‑EU1)

| Сервис | CPU requests | CPU limits | RAM requests | RAM limits | Кол-во реплик |
| :--- | :--- | :--- | :--- | :--- | :--- |
| **search-service** | 4 | 4 | 4 ГБ | 8 ГБ | 2 |
| **booking-service** | 4 | 4 | 4 ГБ | 8 ГБ | 2 |
| **payment-service** | 4 | 4 | 4 ГБ | 8 ГБ | 2 |
| **profile-service** | 4 | 4 | 4 ГБ | 8 ГБ | 2 |
| **review-service** | 4 | 4 | 4 ГБ | 8 ГБ | 2 |
| **recommendation-service** | 4 | 4 | 4 ГБ | 8 ГБ | 2 |
| **kafka-consumer** | 2 | 2 | 2 ГБ | 4 ГБ | 2 |

CPU limits равны CPU requests — для предотвращения throttling при пиках RAM limits удвоены как защита от кратковременных всплесков без риска OOM. Для Go-сервисов устанавливается `GOMEMLIMIT = 90%` от RAM limit, чтобы сборщик мусора активизировался до достижения жёсткого предела контейнера.

**Примечание:** Все численные значения в разделе 11 являются расчётными и могут быть скорректированы после нагрузочного тестирования в реальной среде. Количество серверов для неевропейских регионов уменьшено пропорционально их доле трафика (13,5% и 14,8%), но с сохранением минимальных отказоустойчивых конфигураций (например, минимум 1 узел для Redis и Kafka с репликацией на регион).
