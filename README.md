...
Все сервисы развёртываются в **4 дата-центрах** (DC‑EU1, DC‑EU2, DC‑US1, DC‑AP1) в гибридной инфраструктуре (on‑premises + AWS). Stateless-микросервисы (Golang) работают под управлением Kubernetes; stateful-компоненты (PostgreSQL, Elasticsearch, Redis, Kafka) — на выделенных физических серверах вне оркестрации.

Расчёты выполнены для **Европы (DC‑EU1 + DC‑EU2)** — наиболее загруженного региона с долей **71,7%** трафика, пиковым трафиком **107,6 Гбит/с** и пиковой нагрузкой **3 628 RPS**. Для США (DC‑US1) и Азии (DC‑AP1) количество серверов пересчитано пропорционально их доле трафика (13,5% и 14,8% соответственно) с сохранением минимальных отказоустойчивых конфигураций.

### 11.1 Ресурсные требования сервисов (Европа, пиковая нагрузка)
...
- HAProxy (L7) с SSL‑терминацией → 6 676 CPS/ядро (из расчёта п. 4.2)

| Сервис | Пиковая нагрузка (Европа) | CPU (ядер) | RAM | Примечание |
| :--- | :--- | :--- | :--- | :--- |
| **LVS (L4)** | 107,6 Гбит/с | 4 ядра × 10 серверов = 40 | 40 ГБ | ECMP на коммутаторах, LVS – резерв |
| **HAProxy (L7)** | 3 628 CPS / 107,6 Гбит/с | 8 ядер × 6 серверов = 48 | 48 ГБ | См. п. 4.2 |
| **search-service** | 788 RPS | 1 ядро → с запасом 4 ядра | 8 ГБ | Обращение к Elasticsearch |
| **booking-service** | 78 RPS (запись) + 78 RPS (чтение) → 200 RPS | 1 ядро → 4 ядра | 8 ГБ | Чтение/запись PostgreSQL |
| **payment-service** | 78 RPS запись, 78 RPS чтение → 150 RPS | 1 ядро → 4 ядра | 8 ГБ | ACID, PostgreSQL |
| **profile-service** | 1 814 RPS чтение, 61 RPS запись | 1 ядро → 4 ядра | 8 ГБ | Проверка сессий, чтение users |
| **review-service** | 630 RPS чтение, 1 RPS запись | 1 ядро → 4 ядра | 8 ГБ | Чтение PostgreSQL, запись через Kafka |
| **recommendation-service** | 1 814 RPS чтение из Redis | 1 ядро → 4 ядра | 8 ГБ | Кэш Redis |
| **kafka-consumer** | ~2 300 событий/с (отзывы + бронирования) | 1 ядро → 2 ядра | 4 ГБ | Асинхронные воркеры |
| **PostgreSQL (кластер)** | 1 968 RPS чтения, 60 RPS записи | 192 ядра (весь кластер) | 768 ГБ | 3 шарда × (мастер + 2 реплики) = 9 серверов; на сервер: 64 ядра, 256 ГБ RAM |
| **Elasticsearch** | 788 RPS поиска | 32 ядра | 64 ГБ | Кластер из 3 узлов (16 ядер, 32 ГБ каждый) |
| **Redis** | 1 814 RPS чтения + 1 814 RPS записи + сессии | 16 ядер | 32 ГБ | Redis Cluster: 3 мастера + 3 реплики = 6 узлов |
| **Kafka** | ~2 300 событий/с | 8 ядер | 16 ГБ | 3 брокера, RF=3 |

**Итого CPU для микросервисов в Kubernetes (Европа):**  
...
**Конфигурация Kubernetes узлов (Европа):**  
Узел: 16 vCPU, 64 ГБ RAM. Количество узлов = ceil(34 / 16) = 3 узла + 1 резервный → **4 узла**.

### 11.2 Конфигурации физических серверов (все ДЦ)

| Название | Хостинг | Конфигурация | Ядра | Кол-во (Европа) | Кол-во (США) | Кол-во (Азия) | Всего | Примечание |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- | :--- |
| **kubenode** | on‑prem | 2×AMD EPYC 7282 / 4×16 ГБ / 2×NVMe 1 ТБ / 2×25GbE | 16 | 4 | 2 | 2 | 8 | США и Азия – малая нагрузка |
| **lvs-l4** | on‑prem | 1×Intel Xeon E-2334 / 4×8 ГБ / 1×NVMe 256 ГБ / 2×25GbE | 4 | 10 | 4 | 4 | 18 | См. п. 4.3 |
| **haproxy-l7** | on‑prem | 2×Intel Xeon Silver 4314 / 4×16 ГБ / 2×NVMe 512 ГБ / 2×25GbE | 16 | 6 | 2 | 2 | 10 | См. п. 4.3 |
| **db-postgres** | on‑prem | 2×AMD EPYC 7282 / 8×32 ГБ / 4×NVMe 4 ТБ / 2×25GbE | 32 | 9 | 3 | 3 | 15 | 3 шарда + 2 реплики на регион |
| **db-elasticsearch** | on‑prem | 2×Intel Xeon Silver 4210 / 4×16 ГБ / 4×NVMe 2 ТБ / 2×25GbE | 16 | 3 | 1 | 1 | 5 | RF=2, малые регионы – 1 узел (репликация в EU) |
| **db-redis** | on‑prem | 2×Intel Xeon Silver 4210R / 8×16 ГБ / 2×NVMe 512 ГБ / 2×25GbE | 16 | 6 | 2 | 2 | 10 | Redis Cluster: 3 мастера + 3 реплики (EU), для US/AP – минимально 2 |
| **kafka** | on‑prem | 2×Intel Xeon Silver 4210 / 4×16 ГБ / 4×NVMe 2 ТБ / 2×25GbE | 16 | 3 | 1 | 1 | 5 | RF=3, брокеры в малых регионах синхронизируются с EU |

### 11.3 Аллокация ресурсов в Kubernetes (Европа)
...
CPU limits равны CPU requests – для предотвращения throttling.  
RAM limits удвоены – защита от кратковременных всплесков.  
Для Go-сервисов устанавливается `GOMEMLIMIT = 90%` от RAM limit, чтобы активизировать GC до достижения жёсткого предела контейнера.

**Примечание:** Все численные значения являются расчётными и могут корректироваться после нагрузочного тестирования. Количество серверов в неевропейских регионах уменьшено пропорционально доле трафика (13,5% и 14,8%), но сохранены минимальные отказоустойчивые конфигурации (например, для Kafka – хотя бы 1 брокер с репликацией из EU, для PostgreSQL – минимум 3 сервера для шарда и реплик).
