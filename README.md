...
| **PgBouncer** | Пул соединений к PostgreSQL | • Объединяет множество коротких подключений (3 628 RPS) в ограниченное число долгоживущих сессий<br>• Снижает накладные расходы на создание новых соединений к PostgreSQL<br>• Уменьшает нагрузку на CPU PostgreSQL |
| **Patroni** | Автоматический failover PostgreSQL | • Обеспечивает автоматическое переключение мастера PostgreSQL при падении<br>• Работает с репликами (1 мастер, 2 реплики) без ручного вмешательства<br>• Минимизирует время простоя при отказах |
...
| **Jaeger / OpenTelemetry** | Распределённая трассировка запросов | • Отслеживание пути запроса через микросервисы (HAProxy → сервис бронирований → PostgreSQL → ответ)<br>• Помогает находить узкие места в распределённой системе<br>• Измерение задержек на каждом этапе обработки запроса |
...
| **GitLab CI / GitHub Actions** | CI/CD пайплайны | • Автоматическая сборка, тестирование и деплой микросервисов<br>• Ускорение поставки новых функций (Booking.com делает сотни деплоев в день)<br>• Автоматические проверки качества кода и тесты |
| **WAL-G** | Резервное копирование PostgreSQL | • Ежедневные полные бэкапы + непрерывная архивация WAL<br>• Позволяет восстановить данные на любую секунду (Point-in-Time Recovery)<br>• Сжатие и шифрование бэкапов в S3 |
